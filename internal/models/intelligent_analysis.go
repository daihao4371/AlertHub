...
// IntelligentAnalysisConfig 智能分析配置表 - 支持多租户和分级配置
type IntelligentAnalysisConfig struct {
	ID          uint   `gorm:"primaryKey" json:"id"`                              // 主键ID
	ConfigType  string `gorm:"type:varchar(50);not null;index" json:"configType"` // 配置类型: global/tenant/rule
	ConfigScope string `gorm:"type:varchar(100);index" json:"configScope"`        // 作用域ID (租户ID或规则ID)
	ConfigKey   string `gorm:"type:varchar(100);not null;index" json:"configKey"` // 配置键名
	ConfigValue string `gorm:"type:text" json:"configValue"`                      // 配置值(JSON字符串)
	IsEnabled   bool   `gorm:"default:true" json:"isEnabled"`                     // 是否启用
	Description string `gorm:"type:varchar(255)" json:"description"`              // 配置描述
	CreatedBy   string `gorm:"type:varchar(64)" json:"createdBy"`                 // 创建者
	CreatedAt   int64  `gorm:"autoCreateTime" json:"createdAt"`                   // 创建时间
	UpdatedAt   int64  `gorm:"autoUpdateTime" json:"updatedAt"`                   // 更新时间
}

// TableName 指定表名
func (IntelligentAnalysisConfig) TableName() string {
	return "intelligent_analysis_configs"
}

// IntelligentAnalysisResult 智能分析结果表 - 存储每次分析的详细结果
type IntelligentAnalysisResult struct {
	ID              string  `gorm:"primaryKey;type:varchar(64)" json:"id"`                                   // 分析结果唯一ID
	TenantID        string  `gorm:"type:varchar(32);not null;index" json:"tenantId"`                         // 租户ID
	EventID         string  `gorm:"type:varchar(64);not null;index" json:"eventId"`                          // 告警事件ID
	Fingerprint     string  `gorm:"type:varchar(128);index" json:"fingerprint"`                              // 告警指纹
	RuleID          string  `gorm:"type:varchar(64);index" json:"ruleId"`                                    // 规则ID
	RuleName        string  `gorm:"type:varchar(255)" json:"ruleName"`                                       // 规则名称
	AnalysisType    string  `gorm:"type:varchar(50)" json:"analysisType"`                                    // 分析引擎类型
	AnalysisStatus  string  `gorm:"type:varchar(20);not null;default:'pending';index" json:"analysisStatus"` // 分析状态
	AnalysisResult  string  `gorm:"type:longtext" json:"analysisResult"`                                     // 分析结果摘要
	AnalysisScore   float64 `gorm:"type:decimal(5,2)" json:"analysisScore"`                                  // 分析评分(0-100)
	ConfidenceScore float64 `gorm:"type:decimal(5,2)" json:"confidenceScore"`                                // 置信度评分(0-1)
	KeyFindings     string  `gorm:"type:json" json:"keyFindings"`                                            // 关键发现(JSON数组)
	Recommendations string  `gorm:"type:json" json:"recommendations"`                                        // 建议措施(JSON数组)
	AnalysisContext string  `gorm:"type:json" json:"analysisContext"`                                        // 分析上下文数据(JSON)
	ProcessingTime  int     `json:"processingTime"`                                                          // 处理耗时(毫秒)
	ErrorMessage    string  `gorm:"type:text" json:"errorMessage"`                                           // 错误信息
	CreatedAt       int64   `gorm:"autoCreateTime" json:"createdAt"`                                         // 创建时间
	UpdatedAt       int64   `gorm:"autoUpdateTime" json:"updatedAt"`                                         // 更新时间
	AnalyzedAt      *int64  `json:"analyzedAt"`                                                              // 分析完成时间
}

// TableName 指定表名
func (IntelligentAnalysisResult) TableName() string {
	return "intelligent_analysis_results"
}

// IntelligentAnalysisStatistics 智能分析统计表 - 按日统计分析情况
type IntelligentAnalysisStatistics struct {
	ID                 uint    `gorm:"primaryKey" json:"id"`                            // 主键ID
	TenantID           string  `gorm:"type:varchar(32);not null;index" json:"tenantId"` // 租户ID
	Date               int64   `gorm:"not null;index" json:"date"`                      // 统计日期(Unix时间戳,当日0点)
	TotalAnalyses      int     `gorm:"default:0" json:"totalAnalyses"`                  // 当日总分析次数
	SuccessfulAnalyses int     `gorm:"default:0" json:"successfulAnalyses"`             // 成功分析次数
	FailedAnalyses     int     `gorm:"default:0" json:"failedAnalyses"`                 // 失败分析次数
	AvgProcessingTime  float64 `gorm:"type:decimal(10,2)" json:"avgProcessingTime"`     // 平均处理时间(毫秒)
	AvgAnalysisScore   float64 `gorm:"type:decimal(5,2)" json:"avgAnalysisScore"`       // 平均分析评分
	AnomaliesDetected  int     `gorm:"default:0" json:"anomaliesDetected"`              // 检测到的异常数量
	PatternsIdentified int     `gorm:"default:0" json:"patternsIdentified"`             // 识别的模式数量
	CreatedAt          int64   `gorm:"autoCreateTime" json:"createdAt"`                 // 创建时间
	UpdatedAt          int64   `gorm:"autoUpdateTime" json:"updatedAt"`                 // 更新时间
}

// TableName 指定表名
func (IntelligentAnalysisStatistics) TableName() string {
	return "intelligent_analysis_statistics"
}
