...
INSERT ignore INTO alerthub.rule_templates (
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，ETCD 数据库过大可能导致查询和写入操作的延迟增加，影响 APISIX 的性能。 数据库过大还可能增加 ETCD 集群的负载，导致资源消耗增加，甚至可能触发 OOM（内存溢出）等问题。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，频繁的 leader 变更会导致配置更新延迟和性能下降。 可能引发 ETCD 的可用性问题，从而影响 APISIX 的配置同步和运行稳定性。 可能是由于网络抖动、节点故障或资源瓶颈等问题导致的，需要进一步排查。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当缺少 leader 时，ETCD 集群将无法处理写操作，这会导致 APISIX 无法进行配置更新。 配置无法更新可能导致流量调度、插件配置等关键功能失效，直接影响业务的运行。 这种情况通常是由于 ETCD 集群的严重故障或节点失联导致的，需及时修复。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，APISIX 无法从 Etcd 读取或写入配置信息，可能导致路由规则更新失败、服务发现异常等问题，进而影响整个 API 网关的正常运行。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '!=1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，生产环境APISix服务正在经历高请求速率。每秒超过1000个请求。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，连接数超过apisix工作节点的worker connections总数后，网关会停止工作，新的请求会被拒绝，导致业务受影响。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '容器：${labels.instance}，被意外杀死，请注意排查！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '容器：${labels.instance}，CPU使用率过高，当前：${labels.value}%，请注意排查！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '容器：${labels.instance}，内存使用率过高，当前：${labels.value}%，请注意排查！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '容器：${labels.instance}，存储使用率过高，当前：${labels.value}%，请注意排查！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，慢查询占用较多计算和存储资源，可能导致其他查询任务响应变慢，影响系统的整体性能。 频繁的慢查询会增加 Elasticsearch 集群的负载，导致 CPU、内存和磁盘 I/O 的压力增大，甚至导致集群不稳定。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，高 CPU 使用率可能是由于高查询或写入负载、复杂的查询操作、数据压缩或解压引起的，可能导致节点性能下降、响应时间变长，甚至引发故障。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，内存使用率过高可能导致频繁的垃圾回收 (GC)，增加延迟，甚至导致“out of memory” (OOM) 错误，使节点宕机。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，节点状态不等于1可能是节点出现down机的情况，会影响到整个集群的稳定性。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '!=1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，磁盘空间不足会导致分片无法写入或重新分配，影响集群的正常运行，可能导致集群进入黄色或红色状态。超过 90% 时，集群可能会停止写入。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，写入 QPS 过高可能导致写入延迟增加、集群性能下降，甚至引发写入失败。大量写入请求可能导致分片分配不均、节点负载过高等问题。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，查询 QPS 过高可能导致查询延迟增加、响应时间变长，影响查询性能。集群的查询负载增加，可能造成节点资源紧张。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>50')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，红色状态表示集群中有主分片和副本分片不可用，数据丢失或无法访问。此时，部分或全部数据不可用，可能影响业务的正常运行。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，黄色状态表示集群中的主分片可用，但副本分片有问题，通常是由于资源不足、分片分配失败等原因。虽然数据可以读取，但副本的不可用会降低数据冗余性和容灾能力。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Etcd 中检测到超过 1% 的 HTTP 故障，当前：${labels.value}%',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>0.01')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Etcd 集群应具有奇数个成员，当前成员个数：${labels.value}',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Kafka集群偏移量异常，当前：${labels.value}。可能存在数据异常问题，请观察!',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '< 1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Kafka集群节点缺失，当前：${labels.value}个副本。当一个分区的某个副本所在的 broker 宕机、网络不可达、数据损坏或由于其他原因导致副本不可用时，就会发生副本丢失。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Kafka集群节点缺失，当前：${labels.value}个节点。Kafka 集群中的某个节点下线，可能会导致部分分区数据无法读取或写入，并可能导致消费者或生产者请求失败。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '<3')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Kubernetes API 服务器对于 ${labels.verb} ${labels.resource} 的第 99 个百分位延迟为 ${labels.value} 秒。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点 ${labels.instance}，存在 NetworkUnavailable 情况',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点 ${labels.instance}，超出 pod 容量',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'MongoDB实例：${labels.instance} ，复制延迟过高，当前：${labels.value}。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance} 不健康，如果副本集中有节点健康状态异常，可能会导致数据同步延迟、读写操作失败，甚至可能引发服务不可用的问题。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '!=1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            'promQL', 'mongodb_up',
            'annotations', 'MongoDB实例：${labels.instance} ，节点宕机。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance} 连接数过多，当连接数达到上限时，新的客户端连接请求将被拒绝，进而影响应用程序的正常运行，可能引发严重的系统不可用问题。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>50')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'MySQL实例：${labels.instance}，存在慢查询，当前条数：${labels.value}。慢查询通常表明数据库中存在未优化的 SQL 操作，可能导致查询性能下降，影响整体响应时间。需要通过优化查询语句、添加索引、调整表结构或优化数据库配置来解决慢查询问题。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            'promQL', 'mysql_up',
            'annotations', 'MySQL实例：${labels.instance}，意外宕机。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'MySQL实例：${labels.instance}，发生重启。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '<60')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'MySQL实例：${labels.instance}，连接数过多，当前：${labels.value}。连接数接近上限意味着数据库负载很高，可能导致连接请求被拒绝，影响业务操作。 连接数耗尽时，新请求会被拒绝，导致数据库不可用。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，当前：${labels.value}，高线程数可能导致数据库负载增加，需要检查数据库的并发操作、慢查询、事务等待等问题，优化线程池配置和查询效率。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>60')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Redis实例：${labels.instance}，集群中有太多节点标记为主节点。！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            'promQL', 'redis_up',
            'annotations', 'Redis实例：${labels.instance}，发生宕机，请跟进排查！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Redis实例：${labels.instance}，一些与 Redis 的连接已被拒绝。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Redis实例：${labels.instance}，系统内存不足（> 80％），当前：${labels.value}%。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Redis实例：${labels.instance}，集群没有标记为主节点！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '<1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', 'Redis实例：${labels.instance}，连接数不足（使用率 > 90%），当前：${labels.value}%。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，QPS 的突然增长通常意味着突发的业务流量或异常的请求量。 如果增长过快且超出系统处理能力，可能会导致请求积压，响应时间增加，甚至服务不可用。 持续的高 QPS 可能导致资源耗尽，如 CPU、内存和 I/O 负载增高。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，TPS 的快速增长通常意味着大量的消息发送或消费请求。这可能导致 Broker 的压力增加。 当 TPS 增长过快时，Broker 可能面临资源瓶颈（例如内存、CPU、磁盘 I/O），导致处理效率下降。 突增的 TPS 可能由于异常业务流量或系统故障，需进一步排查原因。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '实例：${labels.instance}，磁盘空间不足可能导致消息无法写入，导致消息积压，甚至服务中断。 当磁盘使用率过高时，文件系统性能也可能下降，影响读写效率。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，CPU使用率过高，当前：${labels.value}%，高 CPU 使用率意味着服务器可能接近处理能力上限，影响性能，导致应用程序响应变慢或服务中断！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80'),
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，TCP连接数过高，当前：${labels.value}%，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，主机网络接口可能接收了太多数据（> 100 MB/s），当前：${labels.value}%，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，内存使用率过高，当前：${labels.value}%，内存紧张可能导致系统频繁进行内存交换（swap），进而引起性能下降。如果内存耗尽，应用程序可能崩溃或无法分配更多资源。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>80'),
                JSON_OBJECT('severity', 'P1', 'expr', '>75'),
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，主机网络接口可能发送了太多数据（> 100 MB/s），当前：${labels.value}%，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>100')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，${labels.mountpoint} 文件系统出现设备错误，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==1')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，检测到时钟偏差，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>=0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            'promQL', 'up{}',
            'annotations', '实例：${labels.instance}，节点状态可能是exporter运行状态异常，也能是服务器宕机，当此告警发生后，需要进一步排查来确保节点的状态。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '==0')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，磁盘可能写入了太多数据 (> 100 MB/s)，当前：${labels.value}%，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>100')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，磁盘可能读取了太多数据 (> 100 MB/s)，当前：${labels.value}%，请尽快处理！',
            'forDuration', 60,
            'rules', JSON_ARRAY(
                JSON_OBJECT('severity', 'P0', 'expr', '>100')
            )
    ...
    (
        ...
        JSON_OBJECT(
            ...
            'annotations', '节点：${labels.instance}，磁盘使用率过高，当前：${labels.value}%，磁盘空间不足会导致文件无法写入、新日志无法记录，甚至可能使服务无法正常运行。尤其在数据增长迅速的系统中，这需要重点监控。',
            'forDuration', 60,
            'rules', JSON_ARRAY(
