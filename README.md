...
<p align="center">
  <b>🌐 AlertHub —— 云原生环境下的轻量级智能监控告警引擎</b>
</p>
...
AlertHub 是一款专为云原生环境设计 的轻量级监控告警引擎，聚焦于可观测性（Metrics、Logs、Traces）与系统稳定性保障，提供从采集、分析到告警的全链路解决方案 。

🔍 **AI 智能加持，让告警更有“洞察力”**

通过 AI 技术深度分析 Metrics、Logs 和 Traces 中的异常信号，精准定位根因，智能生成排查建议与修复方案，显著提升故障响应效率。

![img.png](assets/系统架构图.png)
...
### 官方镜像地址
- **后端服务**: `registry.cn-hangzhou.aliyuncs.com/devops-dh/alerthub:beta-v1`
- **前端服务**: `registry.cn-hangzhou.aliyuncs.com/devops-dh/alerthub-web:beta-v1`

> 💡 **提示**: 使用上述镜像可直接替换 Helm Chart 中的默认镜像,无需自行构建。

## 📦 部署方式

AlertHub 提供多种部署方式,可根据实际环境灵活选择:

### 方式一: Helm Chart 标准版 (推荐新用户)
**适用场景**: 快速体验、测试环境、无现有 MySQL/Redis 的场景

```bash
# 使用标准版 Chart,自动部署 MySQL + Redis + AlertHub 完整环境
cd deploy/helmchart
helm install alerthub . -n monitoring --create-namespace
```

**特点**:
- ✅ 一键部署,包含 MySQL 8.0、Redis 及 AlertHub 全套服务
- ✅ 适合快速体验和测试环境
...
tar -xzf deploy/alerthub-custom-1.0-beta.tar.gz
cd alerthub
...
# 部署(关闭内置 MySQL/Redis)
helm install alerthub . -n monitoring \
...
```bash
helm install alerthub . -n monitoring \
  --set service.image.repository=registry.cn-hangzhou.aliyuncs.com/devops-dh/alerthub \
  --set service.image.tag=beta-v1 \
  --set web.image.repository=registry.cn-hangzhou.aliyuncs.com/devops-dh/alerthub-web \
